...
@Composable
fun TourismDetailScreen(navController: NavController, activityType: String, viewModel: TourismViewModel) {
    val filtered = viewModel.getActivitiesByType(activityType)

    Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
        Text(text = "$activityType Activities", modifier = Modifier.padding(bottom = 16.dp))
        LazyColumn {
            items(filtered) { item ->
